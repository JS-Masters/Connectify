...
import { Avatar, AvatarBadge, Box, Button, Input, List, ListItem } from "@chakra-ui/react";
import {
  blockUser,
  getAllUsers,
  getBlockedUsers,
...
const BlockedUsersPopUp = () => {
  ...
  useEffect(() => {
    if (userData) {
      getBlockedUsers(userData.handle)
        ...
        .then((blockedUsersHandles) => getUsersObjectsByHandles(blockedUsersHandles))
  ...
  const handleInputChange = (event) => {
    if (event.target.value.length) {
      ...
      getUsersObjectsByHandles(filteredUsers)
          ...
          {(close) => (
            <div className="modal" style={{ width: '500px', height: '500px', backgroundImage: "url('/pop-up-background2.jpg')" }}>
              ...
              <div
                ...
                className="content"
              >
                <Input id="search-blocked-users" value={searchTerm} onChange={handleInputChange} />
                <List
                  className="blocked-users-list"
                  overflow="hidden"
                  overflowY="auto"
                  style={{ border: '2px solid black', borderRadius: '10px', height: '180px', padding: '2px', marginTop: '20px' }}>
                  {Boolean(usersBySearchTerm.length) &&
                    usersBySearchTerm.map((user) => (
                      <ListItem
                        className="single-blocked-user"
                        key={v4()}
                        style={{ cursor: "default", border: '1px solid transparent', borderRadius: '5px', padding: '2px', margin: '4px' }}
                      >
                        <Avatar size='sm' src={user.avatarUrl}>
                          <AvatarBadge bg="teal.500" >{<UserStatusIcon userHandle={user.handle} iconSize={'8px'} />}</AvatarBadge>
                        </Avatar>
                        <span className="user-handle-span" style={{ margin: '10px', fontSize: '20px' }}>{user.handle}</span>
                        <Button
                          id="block-button"
                          size='sm'
                          style={{ float: "right", color: "#B00B1D", background: 'transparent', border: "1px solid #B00B1D", marginRight: '10px' }}
                  ...
                  {Boolean(blockedUsersObjects.length) ? (
                    blockedUsersObjects.map((user) => (
                      <ListItem
                        className="single-blocked-user"
                        key={v4()}
                        style={{ cursor: "pointer", border: '1px solid transparent', borderRadius: '5px', padding: '2px', margin: '4px' }}
                      >
                    ...
                    <Box textAlign='center' marginTop='10px' color='rgb(0, 128, 0)' fontWeight='bold'>
